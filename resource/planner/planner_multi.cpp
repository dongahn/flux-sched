                                ...
                                const char **resource_types, size_t len)
{
    ...
    ctx = static_cast<planner_multi_t *> (xzmalloc (sizeof (*ctx)));
    ctx->resource_totals = static_cast<uint64_t *> (
                               xzmalloc (len * sizeof (*(ctx->resource_totals))));
    ctx->resource_types = static_cast<char **> (
                              xzmalloc (len * sizeof (*(ctx->resource_types))));
    ctx->planners = static_cast<planner_t **> (
                        xzmalloc (len * sizeof (*(ctx->planners))));
    ctx->size = len;
    ctx->iter.on_or_after = 0;
    ctx->iter.duration = 0;
    ctx->iter.counts = static_cast<int64_t *> (
                           xzmalloc (len * sizeof (*(ctx->iter.counts))));
    for (i = 0; i < static_cast<int> (len); ++i) {
                       ...
                       planner_multi_t *ctx, const char *resource_type)
{
    ...
    if (!ctx || !resource_type)
        goto done;
    for (i = 0; i < static_cast<int> (ctx->size); i++) {
        if (!strcmp (ctx->resource_types[i], resource_type)) {
            rc = planner_resource_total (ctx->planners[i]);
            break;
        }
    }
    if (i == static_cast<int> (ctx->size))
        errno = EINVAL;
done:
    return rc;
}
...
extern "C" void planner_multi_destroy (planner_multi_t **ctx_p)
{
    int i = 0;
    if (ctx_p && *ctx_p) {
        for (i = 0; i < static_cast<int> ((*ctx_p)->size); ++i) {
                       ...
                       size_t len)
{
    ...
    do {
        unmet = 0;
        for (i = 1; i < static_cast<int> (ctx->size); ++i) {
