...
#
declare -r long_opts="help,nnodes:,ncores-per-node:"
declare -r short_opts="hN:C:"
declare -r usage="
...
if [ ${level} -eq 1 ]; then
    ...
    printf "%-30s %-30s %-30s\n" "NodeShare" "CoreShare" > ${prefix}/perf.${prefix}.out
    printf "%-30s %-30s %-30s\n" ${nnodes}  ${ncores} >> ${prefix}/perf.${prefix}.out
else
    # Intermediate Level
    jobid=""
    jobids=""
...
${next} ${prefix}.${i} > ${prefix}/flux.${prefix}.${i}.json
    ...
    printf "%-30s %-30s %-30s\n" "NodeShare" "CoreShare" > ${prefix}/perf.${prefix}.out
    printf "%-30s %-30s %-30s\n" ${node_share} ${core_share} >> ${prefix}/perf.${prefix}.out
    #
    # End 
    #
fi
